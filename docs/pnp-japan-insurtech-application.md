...
- Target size: **¥30M–¥50M (¥3,000万〜¥5,000万)** · Timing: preparing to raise (round not yet formally open); aiming for an early first close with aligned investors
...
> ラウンド種別＝**シード**（初の機関調達・ポストプロダクト）、調達額＝**¥3,000〜5,000万**で確定。時期＝現在調達準備中・アラインする投資家とは可能な限り早期に一次クローズ（特定四半期があれば差替）。
...
現在シードラウンド（¥3,000〜5,000万）を準備中で、アラインする投資家とは可能な限り早期に一次クローズしたいと考えており、
...
送信前チェック:
- [ ] `【要記入：…】`（氏名・役職・メール）を埋める。シード／¥3,000〜5,000万は確定値。時期＝「現在調達準備中・アラインする投資家とは可能な限り早期に一次クローズ」（正式オープン後に「実施中」へ。誇張しない＝pre-contact と同じ規律）
- [ ] 【記録 2026-05-17】PnP 問い合わせフォームは **縮小前の重い資金使途＋¥3,000〜5,000万＋シード＋「現在調達準備中…」で送信済み**。訂正メールは送らない。金額↔スコープの不整合は **初回面談の第一声＋lean デッキ** で統一（指定工場ネットワーク本格展開・有償本番スケール＝シリーズA）
