...
export type Resource = {
  ...
  badge?: string;
  /**
   * Optional direct download URL. When set, the download starts immediately
...
export function ResourceCard({ resource, delay = 0 }: { resource: Resource; delay?: number }) {
  ...
  function onSubmitted() {
    if (resource.downloadUrl && typeof window !== "undefined") {
      // Small delay so the success pane paints before the browser starts the download.
      setTimeout(() => {
        window.location.href = resource.downloadUrl as string;
      }, 400);
    }
  }
  ...
  return (
    <ScrollReveal variant="fade-up" delay={delay}>
      <div className="h-full rounded-2xl border border-white/[0.08] bg-white/[0.03] p-7 md:p-8 hover:bg-white/[0.06] hover:border-white/[0.14] transition-colors flex flex-col">
        ...
        <h3 className="text-[1.125rem] md:text-[1.25rem] font-bold text-white leading-[1.4]">
          {resource.title}
        </h3>
        <p className="mt-3 text-[0.938rem] leading-[1.75] text-white/55 flex-1">
          {resource.description}
        </p>
        {resource.pageCount && (
          <p className="mt-4 text-xs text-white/35">PDF · 約{resource.pageCount}ページ</p>
        )}
        <button
          type="button"
          onClick={openForm}
