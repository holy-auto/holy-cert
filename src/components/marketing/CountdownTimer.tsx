...
function Digit({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="relative w-16 h-20 sm:w-28 sm:h-32 md:w-32 md:h-36 rounded-xl sm:rounded-2xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center shadow-[0_4px_30px_rgba(0,0,0,0.3)]">
        <span className="text-3xl sm:text-5xl md:text-6xl font-bold tabular-nums text-white">
          {String(value).padStart(2, "0")}
        </span>
      </div>
      <span className="mt-2 sm:mt-3 text-[0.625rem] sm:text-sm text-white/40 font-medium tracking-widest uppercase">
        {label}
      </span>
    </div>
  );
