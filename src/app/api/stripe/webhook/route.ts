...
async function sendShopOrderConfirmationEmail(
  ...
  if (!order) {
    console.warn("webhook: shop confirmation email skipped — order not found", { shopOrderId });
    return;
  }
  ...
  if (!members?.[0]) {
    console.warn("webhook: shop confirmation email skipped — no owner/admin member found", { tenantId });
    return;
  }
  ...
  const email = userData?.user?.email;
  if (!email) {
    console.warn("webhook: shop confirmation email skipped — no email for user", {
      tenantId,
      userId: members[0].user_id,
    });
    return;
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
      <div style="border-bottom: 2px solid #34c759; padding-bottom: 12px; margin-bottom: 20px;">
        <h2 style="margin: 0; color: #1d1d1f; font-size: 18px;">ご注文ありがとうございます</h2>
      </div>
      <p style="color: #1d1d1f; line-height: 1.6;">
        お支払いが完了いたしました。<br>
...
`;
  ...
  const sent = await sendResendEmail({
    to: email,
    reply_to: "support@ledra.co.jp",
    subject: `【Ledra】ご注文を承りました（${order.order_number}）`,
    html,
    text,
    idempotencyKey,
  });
  if (isResendFailure(sent)) {
    console.error("webhook: shop confirmation email send failed", {
      tenantId,
      shopOrderId,
      emailMasked: maskEmail(email),
      status: sent.status,
    ...
    console.info("webhook: shop confirmation email sent", {
      tenantId,
      shopOrderId,
      emailMasked: maskEmail(email),
      resendId: sent.id,
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (session.metadata?.shop_order_id) {
          ...
          if (tenantId) {
            await sendShopOrderConfirmationEmail(supabase, tenantId, shopOrderId, `shop-order-paid:${event.id}`);
          }
