...
const RELEVANT_KEYWORDS = [
  // コーティング
  "コーティング", "coating", "ガラスコーティング", "セラミックコーティング",
  "KeePer", "キーパー", "撥水", "親水", "疎水",
  // PPF
  "PPF", "プロテクションフィルム", "ペイントプロテクション", "XPEL",
  "サンテック", "STEK", "3M", "保護フィルム", "ラッピング",
  // 塗装
  "塗装", "板金", "鈑金", "ボデー", "ボディ", "ペイント",
  "補修", "調色", "カラー",
  // 整備
  "整備", "車検", "点検", "メンテナンス", "修理",
  "整備士", "認証工場", "指定工場",
  // カーディテイリング
  "ディテイリング", "detailing", "磨き", "研磨", "ポリッシュ",
  "洗車", "カーケア",
  // 業界・市場
  "アフターマーケット", "施工店", "施工", "認定", "資格",
  "損害保険", "保険", "査定", "事故車",
  // EV関連
  "EV", "電気自動車", "テスラ", "BYD",
  // 法改正・規制
  "法改正", "改正", "省令", "告示", "通達", "パブリックコメント",
  "道路運送車両法", "保安基準", "車両法", "型式指定",
  "OBD検査", "特定整備", "電子制御",
  "排ガス規制", "騒音規制", "燃費基準", "CAFE",
  "自賠責", "リコール", "届出",
  // 海外動向
  "EU規制", "欧州", "米国", "北米", "NHTSA", "EPA",
  "中国", "IATF", "ISO", "グローバル",
  "SEMA", "Automechanika", "IDA",
  "paint protection", "ceramic coating", "detailing industry",
  "aftermarket", "auto body", "collision repair",
];

// ── RSS フィード ──
const RSS_FEEDS = [
  { url: "https://tosojiho.jp/?feed=rss2", source: "日本塗装時報", category: "塗装・コーティング", alwaysRelevant: true },
  ...
  { url: "https://car.watch.impress.co.jp/data/rss/1.0/car/feed.rdf", source: "Car Watch", category: "自動車ニュース", alwaysRelevant: false },
  ...
  // ── 法改正・行政 ──
  { url: "https://www.mlit.go.jp/rss/road_bureau.xml", source: "国交省（道路局）", category: "法改正・規制", alwaysRelevant: false },
  // ── 海外ニュース ──
  ...
  { url: "https://www.greencarreports.com/rss", source: "Green Car Reports", category: "海外EV動向", alwaysRelevant: false },
  { url: "https://feeds.feedburner.com/Autoblog", source: "Autoblog", category: "海外動向", alwaysRelevant: false },
  { url: "https://www.bodyshopbusiness.com/feed/", source: "BodyShop Business", category: "海外・板金塗装", alwaysRelevant: true },
  { url: "https://www.fenderender.com/feed/", source: "Fender Bender", category: "海外・板金塗装", alwaysRelevant: true },
];
...
async function scrapesite(target: ScrapeTarget): Promise<ScrapedArticle[]> {
  ...
  $(target.selectors.articleList).each((_, el) => {
    ...
    if (target.selectors.date) {
      ...
      if (dateMatch) {
        dateStr = new Date(`${dateMatch[1]}-${dateMatch[2].padStart(2, "0")}-${dateMatch[3].padStart(2, "0")}`).toISOString();
      }
    }
