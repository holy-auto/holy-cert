...
export async function GET() {
  try {
    ...
    const [
      ...
      admin.from("certificates").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "active"),
      admin.from("certificates").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "void"),
      admin.from("certificates").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "draft"),
      admin.from("certificates").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).gte("created_at", thisMonthStart),
      admin.from("certificates").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "active").gte("expiry_date", today).lte("expiry_date", next7daysStr),
      admin.from("certificates").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "active").gte("expiry_date", today).lte("expiry_date", next30daysStr),
      // Customers: 2 counts
      admin.from("customers").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId),
      admin.from("customers").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).gte("created_at", thisMonthStart),
      // Invoices: total, unpaid (sent+overdue), overdue only
      admin.from("documents").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("doc_type", "invoice"),
      admin.from("documents").select("total").eq("tenant_id", tenantId).eq("doc_type", "invoice").in("status", ["sent", "overdue"]),
      admin.from("documents").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("doc_type", "invoice").eq("status", "overdue"),
      // Reservations: 3 counts
      admin.from("reservations").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("scheduled_date", today).neq("status", "cancelled"),
      admin.from("reservations").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).gte("scheduled_date", weekStartStr).lte("scheduled_date", weekEndStr).neq("status", "cancelled"),
      admin.from("reservations").select("*", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("status", "confirmed"),
      // Orders: 2 counts
      admin.from("job_orders").select("*", { count: "exact", head: true }).or(`from_tenant_id.eq.${tenantId},to_tenant_id.eq.${tenantId}`).in("status", ["pending", "accepted", "in_progress", "approval_pending", "payment_pending"]),
      admin.from("job_orders").select("*", { count: "exact", head: true }).or(`from_tenant_id.eq.${tenantId},to_tenant_id.eq.${tenantId}`).eq("status", "completed").gte("updated_at", thisMonthStart),
    ]);
