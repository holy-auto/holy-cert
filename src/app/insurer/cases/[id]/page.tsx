  ...
  if (!caseData) {
    return (
      <div className="mx-auto max-w-5xl p-6">
        <Link href="/insurer/cases" className="text-sm text-blue-600 hover:underline">
          &larr; 案件一覧へ
        </Link>
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusClasses(caseData.status)}`}
              >
                {STATUS_MAP[caseData.status]?.label ?? caseData.status}
              </span>
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${priorityClasses(caseData.priority)}`}
              >
                {PRIORITY_MAP[caseData.priority]?.label ?? caseData.priority}
              </span>
            </div>
          </div>
      ...
      <section className="rounded-2xl border border-border-default bg-surface p-6">
        ...
        <dl className="grid gap-4 sm:grid-cols-2">
          <div>
            <dt className="text-xs font-medium text-muted">案件番号</dt>
            <dd className="mt-0.5 font-mono text-sm text-primary">{caseData.case_number}</dd>
          </div>
          <div>
            <dt className="text-xs font-medium text-muted">ステータス</dt>
            <dd className="mt-0.5">
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusClasses(caseData.status)}`}
              >
          ...
          <div>
            <dt className="text-xs font-medium text-muted">優先度</dt>
            <dd className="mt-0.5">
              <span
                className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${priorityClasses(caseData.priority)}`}
              >
          ...
          <div>
            <dt className="text-xs font-medium text-muted">カテゴリ</dt>
            <dd className="mt-0.5 text-sm text-primary">{caseData.category || "-"}</dd>
          </div>
          <div>
            <dt className="text-xs font-medium text-muted">作成日</dt>
            <dd className="mt-0.5 text-sm text-primary">{formatDateTime(caseData.created_at)}</dd>
          </div>
          <div className="sm:col-span-2">
            <dt className="text-xs font-medium text-muted">説明</dt>
            <dd className="mt-0.5 whitespace-pre-wrap text-sm text-primary">{caseData.description || "-"}</dd>
          </div>
        </dl>
      </section>
      ...
      <section className="rounded-2xl border border-border-default bg-surface">
        <div className="border-b border-border-default px-6 py-4">
          <h2 className="text-lg font-bold text-primary">メッセージ</h2>
        </div>
        <div className="h-96 overflow-y-auto p-6 space-y-4">
          {messages.length === 0 ? (
            <p className="text-center text-sm text-muted">メッセージはまだありません</p>
          ) : (
            messages.map((msg) => {
              ...
              if (isSystem) {
                return (
                  <div key={msg.id} className="flex justify-center">
                    <div className="max-w-md rounded-xl bg-amber-50 px-4 py-2 text-center">
                      <span className="mr-2 inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700">
                        システム
                      </span>
                      <p className="mt-1 text-sm text-amber-800">{msg.content}</p>
                      <p className="mt-1 text-[10px] text-amber-500">{formatDateTime(msg.created_at)}</p>
                    </div>
                  </div>
                );
              }
              return (
                <div key={msg.id} className={`flex ${isInsurer ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[70%] rounded-xl px-4 py-3 ${isInsurer ? "bg-blue-50" : "bg-inset"}`}>
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${isInsurer ? "bg-blue-100 text-blue-700" : "bg-surface-active text-secondary"}`}
                    >
                      {isInsurer ? "保険会社" : "施工店"}
                    </span>
      ...
      <section className="rounded-2xl border border-border-default bg-surface p-6">
        ...
        {attachments.length === 0 ? (
          <p className="text-sm text-muted">添付ファイルはありません</p>
        ) : (
...
const RISK_STYLE: Record<string, { badge: string; label: string }> = {
  high: { badge: "bg-red-400/10 text-red-400 border-red-400/30", label: "高リスク" },
  medium: { badge: "bg-amber-100 text-amber-700 border-amber-200", label: "中リスク" },
  low: { badge: "bg-warning-dim text-warning border-warning/30", label: "低リスク" },
  clear: { badge: "bg-emerald-400/10 text-emerald-500 border-emerald-400/30", label: "問題なし" },
};

const FLAG_JP: Record<string, string> = {
  duplicate_claim: "重複請求",
  velocity_spike: "短期間に多数の案件",
  round_amount: "キリのいい金額",
  same_day_multi: "同日同車両で複数件",
  certificate_void: "証明書が無効",
...
function FraudCheckSection({ caseId }: { caseId: string }) {
  ...
  const style = result ? (RISK_STYLE[result.risk_level] ?? RISK_STYLE.clear) : null;

  return (
    <section className="rounded-2xl border border-border-default bg-surface p-6 space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="text-xs font-semibold tracking-[0.18em] text-muted">AI FRAUD DETECTION</div>
          <h2 className="text-lg font-bold text-primary">不正リスク審査</h2>
        </div>
        <button
          onClick={runCheck}
          disabled={loading}
          ...
          {result.flags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {result.flags.map((f) => (
                <span
                  key={f}
                  className="rounded-full border border-warning/30 bg-warning-dim px-3 py-1 text-xs font-medium text-warning"
                >
                  {FLAG_JP[f] ?? f}
                </span>
              ))}
