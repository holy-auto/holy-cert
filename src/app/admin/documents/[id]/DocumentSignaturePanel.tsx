 ...
 *
 * 帳票詳細ページ（管理画面）に表示する電子署名依頼パネル。
 * 誓約書・同意書など帳票に対して電子署名を依頼するためのUI。
 */
...
export default function DocumentSignaturePanel({ documentId }: Props) {
    ...
    return (
      <div className="rounded-2xl border border-blue-500/30 bg-blue-500/5 p-4 space-y-4">
        ...
        <div className="rounded-xl bg-base p-3 space-y-2">
          <p className="text-xs text-muted">署名リンク（有効期限: 72時間）</p>
        ...
        <p className="text-xs text-muted leading-relaxed">
          このURLを相手方にメール・LINE等で共有してください。
          相手方がURLを開き「署名する」ボタンを押すと電子署名が完了します。
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-blue-500/30 bg-blue-500/5 p-4 space-y-4">
      ...
      <p className="text-xs text-muted leading-relaxed">
        署名者の情報を入力すると署名URLが発行されます。
        URLを相手方に共有することで電子署名（電子署名法第2条）を取得できます。
      </p>

      <div>
        <label className="block text-xs text-muted mb-1">署名者名（任意）</label>
        <input
          type="text"
          value={signerName}
      ...
      <div>
        ...
        <p className="text-xs text-muted mt-1">
          ※現在はURLを手動で共有してください（通知機能は近日実装予定）
        </p>
      </div>
