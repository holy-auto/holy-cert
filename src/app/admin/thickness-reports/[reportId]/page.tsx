...
export default async function StandaloneThicknessReportDetailPage({
  params,
}: {
  ...
  const { data: report } = await supabase
    ...
    .select(
      "id, name, measured_at, calibration_at, device_serial_number, brand, model, vin, year, type_of_body, capacity, power, fuel_type, unit_of_measure, comment, vehicle_id, vehicle:vehicles(id, maker, model, plate_display, vin_code)",
    )
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Link href="/admin/thickness-reports" className="btn-secondary">
            ← レポート一覧に戻る
          </Link>
        }
      />
      ...
      <section className={`glass-card p-5 ${linkedVehicle ? "" : "border border-warning/30"}`}>
        ...
        {linkedVehicle ? (
          <div className="flex items-center gap-3 text-sm">
            ...
            <Link href={`/admin/vehicles/${linkedVehicle.id}`} className="text-accent hover:underline">
              {[linkedVehicle.maker, linkedVehicle.model].filter(Boolean).join(" ") ||
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center rounded-full border border-warning/30 bg-warning-dim px-2 py-0.5 text-[11px] font-semibold text-warning-text">
                未紐付け
              </span>
              <span className="text-secondary">このレポートはLedra内のいずれの車両とも紐付いていません。</span>
            </div>
            {report.vin && (
            ...
            {!report.vin && (
              <div className="text-xs text-muted">
                NexPTG側でVINが設定されていません。NexPTGアプリ側で車両情報にVINを入力して再同期してください。
              </div>
            )}
          </div>
        )}
