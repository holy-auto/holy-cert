...
export default function SecurityClient({ initialTotpFactors }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      await Promise.all(leftovers.map((f) => supabase.auth.mfa.unenroll({ factorId: f.id })));
  ...
  async function unenrollFactor(factorId: string) {
    if (!confirm("2 要素認証を解除します。解除後はパスワードのみでログインできるようになります。よろしいですか？")) {
      return;
    }
    setBusy(true);
    setErr(null);
  ...
  return (
    <div className="space-y-5">
      <FirstUseInlineGuide
        storageKey="settings_security"
        title="2要素認証 (2FA) を有効にする"
        description="保険会社や顧客の個人情報を扱うため、ログイン時の本人確認を強化することを推奨します。Google Authenticator や 1Password などの認証アプリで設定できます。"
        steps={[
          {
            title: "認証アプリを準備",
            description: "Google Authenticator / 1Password / Authy などのアプリをスマホにインストールします。",
          },
          {
            title: "QRコードを読み込み",
            description: "下に表示されるQRを認証アプリで読み取り、6桁のコードを Ledra に入力して有効化。",
          },
          {
            title: "次回以降のログイン",
            description:
              "メール+パスワードに加えて認証アプリの6桁コードが必要になります。コード忘れ時はリカバリーから運営に連絡。",
          },
        ]}
      />
      <section className="glass-card p-5 space-y-5">
        <div>
          <div className="text-xs font-semibold tracking-[0.18em] text-muted">TWO-FACTOR AUTH</div>
          <div className="mt-1 text-base font-semibold text-primary">2 要素認証 (TOTP)</div>
          <p className="mt-1 text-xs text-muted">
            Google Authenticator / 1Password / Authy 等の認証アプリで生成される 6
            桁コードを使って、ログイン時の本人確認を強化します。
          </p>
        </div>
        ...
        {!verifiedFactor && !enroll && (
          <div className="space-y-3 rounded-xl border border-border-default bg-base p-4">
            <div className="text-sm font-semibold text-primary">2 要素認証を有効化する</div>
            <div className="space-y-1">
              <label className="text-xs text-muted">デバイス名 (任意 / 「iPhone 15」「業務用 iPad」など)</label>
              <input
                type="text"
                value={friendlyName}
            ...
            <button type="button" className="btn-primary text-sm" onClick={startEnroll} disabled={busy}>
        ...
        {enroll && (
          <div className="space-y-4 rounded-xl border border-accent/20 bg-accent-dim/40 p-4">
            <div className="text-sm font-semibold text-primary">認証アプリで QR コードをスキャンしてください</div>
            <div className="flex flex-col lg:flex-row gap-4 items-start">
              ...
              <div className="space-y-2 text-xs text-secondary flex-1">
                ...
                <div className="text-muted">登録後、認証アプリに表示される 6 桁コードを入力してください:</div>
                <input
                  type="text"
                  inputMode="numeric"
                  pattern="[0-9]{6}"
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button type="button" className="btn-ghost text-sm" onClick={cancelEnroll} disabled={busy}>
                    キャンセル
                  </button>
                </div>
              </div>
        ...
        {verifiedFactor && !enroll && (
          <div className="space-y-3 rounded-xl border border-border-default bg-base p-4">
            <div className="text-sm text-secondary">
              認証デバイスを紛失した場合や、使用しているアプリを変更する場合は 一度解除して再登録してください。
            </div>
            <button
              type="button"
